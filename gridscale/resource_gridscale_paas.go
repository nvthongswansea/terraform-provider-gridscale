...
func resourceGridscalePaaSServiceRead(d *schema.ResourceData, meta interface{}) error {
	...
	paas, err := client.GetPaaSService(d.Id())
	if err != nil {
	...
	//Get resource limits
	resourseLimits := make([]interface{}, 0)
	for _, value := range props.ResourceLimits {
		limit := map[string]interface{}{
			"resource": value.Resource,
			"limit":    value.Limit,
		}
		resourseLimits = append(resourseLimits, limit)
	}
	if err = d.Set("resource_limit", resourseLimits); err != nil {
...
func resourceGridscalePaaSServiceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	response, err := client.CreatePaaSService(requestBody)
	if err != nil {
		return err
	}
...
func resourceGridscalePaaSServiceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	err := client.UpdatePaaSService(d.Id(), requestBody)
	if err != nil {
		return err
	}
