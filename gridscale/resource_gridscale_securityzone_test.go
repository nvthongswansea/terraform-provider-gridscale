		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDataSourceGridscaleSecurityZoneExists("gridscale_securityzone.foo", &object),
					resource.TestCheckResourceAttr(
						"gridscale_securityzone.foo", "name", name),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDataSourceGridscaleSecurityZoneExists("gridscale_securityzone.foo", &object),
					resource.TestCheckResourceAttr(
						"gridscale_securityzone.foo", "name", "newname"),
				),
			},
		},
	})
...
func testAccCheckDataSourceGridscaleSecurityZoneExists(n string, object *gsclient.PaaSSecurityZone) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		id := rs.Primary.ID
		foundObject, err := client.GetPaaSSecurityZone(id)
		if err != nil {
			return err
		}
...
func testAccCheckDataSourceGridscaleSecurityZoneDestroyCheck(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.GetPaaSSecurityZone(rs.Primary.ID)
		if err != nil {
...
func testAccCheckDataSourceGridscaleSecurityZoneConfig_basic(name string) string {
	return fmt.Sprintf(`
resource "gridscale_securityzone" "foo" {
  name = "%s"
}
`, name)
}
